...
def main():
    ...
    # Use only image data
    image_data = list(
        filter(lambda p: p if p["media_type"] == "image" else None, non_copyright_data)
    )
    ...
    for d in image_data:
        ...
        for image in image_data:
            ...
            # Save all the images
            list(map(lambda k: save_image_from_url(k, download_dir), list(map(lambda k: image[k], found_keys))))
